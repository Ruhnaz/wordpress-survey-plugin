    ...
    <h2><a class="button" name="but_submit" href="?page=SMP_Question&action=add" style="background-color: #00CC33; color:#FFFFFF"/>New</a>&nbsp;&nbsp;<!--<input type="button" value="Publish" class="button"/>&nbsp;&nbsp;<input type="button" value="Trash" class="button"/>--></h2>
    <!-- Form -->
      ...
      <thead>
        <tr>
         ...
         <th id="cb" class="manage-column column-cb check-column" scope="col"><input type="checkbox" /></th>
           ...
           <th id="columnname" class="manage-column column-columnname" scope="col">Section</th>
           <th id="columnname" class="manage-column column-columnname" scope="col">Survey</th>
           <th id="columnname" class="manage-column column-columnname" scope="col">Sequence</th>
    ...
    $date = $dt->format('d-m-Y');
        ...
        <td class="column-columnname"><input type="checkbox" /></td><td class="column-columnname"><a href="#" style="text-decoration:none"><?php echo $row->name; ?></a> <div class="row-actions">
                        <span><a href=?page=SMP_Question&action=edit&id=<?php echo $row->id?>>Edit</a> </span>
                        <span><a href=?page=SMP_Question&action=delete&id=<?php echo $row->id?>>Delete</a> </span>
       ...
       $row_class="alternate";
          $tblname = 'smp_section';
          $wp_track_table = $table_prefix . "$tblname";
          $que="SELECT * FROM {$wp_track_table} where id = $row->section_id;";
          $i=0;
          $r = $wpdb->get_results($que);// print_r($result);
          ?>
                   
                    <td class="column-columnname"><?php echo $r[0]->name;?></td>
                    <?php  global $table_prefix, $wpdb;
       $row_class="alternate";
          $tblname = 'smp_survey';
          $wp_track_table = $table_prefix . "$tblname";
         
          $que="SELECT * FROM {$wp_track_table} where surveyid = $row->survey_id;";
          $i=0;
          $r1 = $wpdb->get_results($que);// print_r($result);
          ?>
                    <td class="column-columnname"><?php echo $r1[0]->name;?></td>
                    <td class="column-columnname"><?php echo $row->sequence; ?></td>
                    <td class="column-columnname"><?php if($status=='1') echo "Active"; else if($status=='0') echo "Inactive";?></td>
                    
                    <td class="column-columnname"><?php echo $date;?></td>
