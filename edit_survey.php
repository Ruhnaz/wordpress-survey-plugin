...
global $wpdb;     
 ...
 $wpdb->update($table_name, array('status' => $status, 'name' => $name,'updated_date' =>$updated_date,'updated_by' =>$updated_by),array('surveyid' => $id)); 
...
<?php 
     ...
     global $table_prefix, $wpdb;
      ...
      $que="SELECT * FROM {$wp_track_table} WHERE surveyid =$id";
...
<!-- Form -->
  <table class="widefat fixed" cellspacing="0" border="0">
    <tr>
      ...
      <td><input type='text' name="id" disabled="disabled" value="<?php echo $result[0]->surveyid?>" class="input_text" />
      <input type='hidden' name="id" value="<?php echo $result[0]->surveyid?>" />
      </td>
    </tr>
     <tr>
      <td>Survey Name *</td>
